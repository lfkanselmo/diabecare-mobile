...
Estado real a la fecha (2026-07-11) — implementado en `diabecare-api`:

1. **IDs generados por el cliente — ✅ hecho**, para las 5 entidades creables offline: `GlucoseReading`, `MealEntry`/`MealItem`, `VitalSign`, `ExerciseLog`, `Medication` (esta última se sumó en Fase 2 — no tenía `createWithId` hasta entonces). Cada `create(...)` estático delega a un `createWithId(id, ...)` que honra un UUID explícito; los use cases de registro aceptan un `clientXxxId` opcional en su `Command`, y los controllers lo exponen como campo opcional en el request (`readingId`, `mealId`/`mealItemId`, `vitalId`, `exerciseId`, `medicationId`). Si no se envía, el servidor genera el ID como siempre (compatibilidad total con el frontend web, que no lo envía). Reenviar el mismo ID es idempotente por construcción (JPA `save()` sin `@GeneratedValue` hace upsert por ID).
...
3. **Endpoints de sincronización incremental — ✅ hecho para los 5 dominios.** Endpoint `/sync` dedicado por recurso (no reutiliza `/history`, que pagina por fecha de *medición* para la UI web — mezclarlo con un cursor de *modificación* habría sido confuso). Implementado primero para glucosa (Fase 1), y replicado en Fase 2 para `meals` (`GET /api/v1/nutrition/{patientId}/meals/sync`), `vitals`, `exercise` y `medications` — mismo patrón exacto (repo query `findByPatientIdAndUpdatedAtAfterOrderByUpdatedAtAsc` + puerto + use case `SyncXxxUseCase` + controller). El de `medications` incluye deliberadamente los desactivados (a diferencia de `GET /{patientId}`, que solo lista activos), porque el cliente offline necesita enterarse de la desactivación. Gap encontrado y corregido en Fase 2: `MealEntry`/`VitalSign`/`ExerciseLog` nunca exponían `updatedAt` en su domain model ni en su response (a diferencia de glucosa) — sin eso, `pullChanges()` del móvil no podía poblar `serverUpdatedAt` al traer cambios remotos.
...
Todos los tests nuevos pasan, suite completa verificada tras cada cambio.
