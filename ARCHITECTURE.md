...
Estos NO están implementados todavía — son groundwork que hay que construir en `diabecare-api` antes o durante el desarrollo del móvil:

1. **IDs generados por el cliente.** Hoy `GlucoseReading.create()` y equivalentes generan el `UUID` en el servidor. Para que el modo offline funcione (el paciente necesita ver su lectura en la lista inmediatamente, antes de sincronizar), el cliente debe generar el UUID y el backend debe aceptarlo en la creación (`INSERT ... ON CONFLICT DO NOTHING` o verificación de idempotencia por ID) en vez de generarlo siempre él mismo. Aplica a glucosa, comidas, signos vitales, ejercicio — cualquier entidad que se pueda crear offline.
2. **Registro de tokens FCM.** Nuevo `MobilePushTokenPort`/adaptador, estructuralmente paralelo al `PushSubscriptionPort` ya existente (Web Push) pero para un `deviceToken` (string opaco de FCM) + `platform` (`ANDROID`/`IOS`) en vez de `endpoint`/`p256dh`/`auth`. `PushNotificationService` necesita una segunda rama de envío (FCM Admin SDK o HTTP v1 API) además de la que ya tiene para Web Push — ambas conviven, un paciente puede tener suscripciones de ambos tipos si usa web y móvil.
3. **Endpoints de sincronización incremental.** La mayoría de entidades (`GlucoseReadingEntity`, `MealEntryEntity`, etc.) ya tienen `updated_at` — bien, es la base necesaria. Falta exponer un parámetro `updatedSince` en los endpoints de listado (o un endpoint `/sync` dedicado por recurso) para que el motor de sync pida solo lo que cambió desde el último cursor guardado, en vez de traer todo el historial en cada sincronización.
4. **Idempotencia en escrituras.** Con reintentos automáticos de red en el motor de sync, una misma mutación podría enviarse dos veces. Con IDs generados por el cliente (punto 1) esto ya queda parcialmente resuelto (un segundo POST con el mismo ID es un no-op), pero vale la pena revisar caso por caso cada endpoint de escritura.

Ninguno de estos cuatro puntos es grande individualmente, pero son prerequisito real antes de que el motor de sync offline (sección 4) tenga algo funcional contra qué sincronizar. Recomendación: abordarlos como una fase de preparación en `diabecare-api` antes de escribir la primera pantalla del móvil, no en paralelo.
