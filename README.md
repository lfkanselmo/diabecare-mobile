...
**De Fase 6 (beta y publicación) ya quedaron listas las partes que sí son tareas de código**: pantalla de política de privacidad pública dentro de la app (`/legal/privacy`, enlazada de forma tappable desde el registro y el perfil — antes era texto plano sin enlace), declaraciones de App Privacy (Apple)/Data Safety (Google Play) redactadas a partir de una auditoría real del código (`PRIVACY_DECLARATIONS.md`), y un scaffold funcional de Fastlane para ambas plataformas (`android/fastlane/`, `ios/fastlane/`). El resto de Fase 6 — TestFlight/Internal Testing, firma real de las apps, screenshots de store, publicación — necesita cuentas reales de desarrollador (Apple/Google) y un Mac para iOS, ninguno de los cuales existe en esta máquina; el detalle completo de qué falta y por qué está en `RELEASE_CHECKLIST.md`.

No se pudo probar la app en un emulador/dispositivo real en esta máquina en ningún momento del proyecto (sin Android SDK; iOS no se puede compilar desde Windows) — todos los contratos (auth, glucosa, nutrición, vitales, ejercicio, medicamentos, ciclo menstrual, reportes, cuidadores, cuenta, administración) se validaron en vivo contra el backend real vía `curl`, y el motor de sync se endureció a nivel de tests unitarios (dobles de conectividad/red), no de validación de campo en dispositivos de gama baja reales.
...
- **[`RELEASE_CHECKLIST.md`](./RELEASE_CHECKLIST.md)** — qué falta para publicar en las stores y por qué no se puede hacer desde esta máquina.
- **[`PRIVACY_DECLARATIONS.md`](./PRIVACY_DECLARATIONS.md)** — respuestas ya redactadas para los formularios de App Privacy (Apple) y Data Safety (Google Play).
