...
Siguiente paso: Fase 5 (hardening del motor offline — conflictos de edición concurrente, pruebas exhaustivas de sincronización). No se pudo probar la app en un emulador/dispositivo real en esta máquina (sin Android SDK; iOS no se puede compilar desde Windows) — todos los contratos (auth, glucosa, nutrición, vitales, ejercicio, medicamentos, ciclo menstrual, reportes, cuidadores, cuenta, administración) se validaron en vivo contra el backend real vía `curl`. Por la misma razón, la auditoría de accesibilidad de Fase 4 quedó acotada a lo verificable por código (Dynamic Type, etiquetas semánticas) — no se pudo probar con VoiceOver/TalkBack activados.
